...
interface IStorageFS extends IStorage{ // FS storage interface
    readFileFS: () => Promise<string>
    writeFileFS: (data: string) => Promise<void>
}
interface IStorageBD extends IStorage{ // BD storage interface
    getTable: () => void
}
class StorageFs implements IStorageFS { // FS storage class
    ...
    insert = async (todo: ITodo) => {
        try {
            if(!(await this.readFileFS()).length) {
                todo['id'] = 1 
                await fs.appendFile(this.pathFile, JSON.stringify([todo], null, 2))
                .then(() => {
                    console.error('\x1b[35m', `The file along the path ${this.pathFile} was successfully created`, '\x1b[0m')
                })
            } else {
                let data = JSON.parse(await this.readFileFS()) as Array<ITodo>
...
todo = new Proxy(todo, {
    set: (target: any, key: keyof ITodo, val: any): boolean => {
        ...
        if(typeof(val) !== typeof(target[key])) {
            throw console.error('\x1b[31m', `${val} should be a type ${typeof(target[key])}, but you entered the type ${typeof(val)}`, '\x1b[0m')
        }
